package com.afiab.scaffolding.server.handler;
...
import com.afiab.scaffolding.server.utils.json.JsonUtils;
...
//        }
        ...
        String userCode = request.getHeader(SysConstants.USER_CODE);
        String userName = request.getHeader(SysConstants.USER_NAME);
        if (StringUtils.isNotBlank(userCode)){
            request.setAttribute(SysConstants.USER_CODE, userCode);
            RequestHolder.set(SysConstants.USER_CODE, userCode);
        }
        if (StringUtils.isNotBlank(userName)) {
            ...
            RequestHolder.set(SysConstants.USER_NAME, userName);
        }
        ...
        if(Objects.isNull(RequestHolder.getCurrentUserCode())||
            ...
            String authStr = request.getHeader(SysConstants.AUTHORIZATION);
            log.info("authStr-验证未通过!!! ,authStr : [ {} ],userName:[{}],userCode:[{}]", authStr,userName,userCode);
            response.setCharacterEncoding("UTF-8");
            response.setContentType("application/json; charset=utf-8");
            response.getWriter().write(JsonUtils.obj2String(R.error(BusinessEnumCode.HEADER_INFO_NOT_FOUND)));
            return false ;
        }

        log.info("LoginInterceptor.preHandle, token:{}, userCode:{}, userName:{}, reqHeader UserCode: {}", token , userCode , userName , request.getAttribute(SysConstants.USER_CODE));
        return true;
    }
